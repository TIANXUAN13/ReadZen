...
def save_uploaded_article(title, author, content, file_name='', file_size=0):
    ...
    conn = get_conn()
    cur = conn.cursor()
    cur.execute('''INSERT INTO uploaded_articles (title, author, content, file_name, file_size)
                   VALUES (?, ?, ?, ?, ?)''',
...
def get_uploaded_articles():
    """获取所有上传的文章"""
    conn = get_conn()
    rows = conn.execute('SELECT * FROM uploaded_articles ORDER BY date_added DESC').fetchall()
    conn.close()
    return [dict(r) for r in rows]
